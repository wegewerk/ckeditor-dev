...
CKEDITOR.dialog.add( 'linkfe', function( editor ) {
	var plugin = CKEDITOR.plugins.linkfe;
	...
	var targetChanged = function() {
			...
			switch ( value ) {
				case 'frame':
					targetName.setLabel( editor.lang.linkfe.targetFrameName );
					targetName.getElement().show();
					break;
				case 'popup':
					popupFeatures.show();
					targetName.setLabel( editor.lang.linkfe.targetPopupName );
					targetName.getElement().show();
					break;
				default:
	...
	var parseLink = function( editor, element ) {
			...
			if ( element ) {
				...
				CKEDITOR.plugins.linkfe.synAnchorSelector ? retval.adv.advCSSClasses = getLinkClass( element ) : advAttr( 'advCSSClasses', 'class' );
			...
			if ( CKEDITOR.plugins.linkfe.emptyAnchorFix ) {
			...
			if ( CKEDITOR.plugins.linkfe.fakeAnchor ) {
				var imgs = editor.document.getElementsByTag( 'img' );
				for ( i = 0, count = imgs.count(); i < count; i++ ) {
					if ( ( item = CKEDITOR.plugins.linkfe.tryRestoreFakeAnchor( editor, imgs.getItem( i ) ) ) )
	...
	return {
		...
		onOk: function() {
			...
			if ( !this._.selectedElement ) {
				...
				if ( data.adv && data.adv.advName && CKEDITOR.plugins.linkfe.synAnchorSelector )
