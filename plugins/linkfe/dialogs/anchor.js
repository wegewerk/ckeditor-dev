...
CKEDITOR.dialog.add( 'anchor', function( editor ) {
	...
	return {
		title: editor.lang.linkfe.anchor.title,
		minWidth: 300,
		minHeight: 60,
		onOk: function() {
		...
		onShow: function() {
			...
			if ( fullySelected ) {
				if ( CKEDITOR.plugins.linkfe.fakeAnchor ) {
					var realElement = CKEDITOR.plugins.linkfe.tryRestoreFakeAnchor( editor, fullySelected );
				...
				partialSelected = CKEDITOR.plugins.linkfe.getSelectedLink( editor );
				if ( partialSelected ) {
		...
		contents: [
			{
			id: 'info',
			label: editor.lang.linkfe.anchor.title,
			accessKey: 'I',
			elements: [
				{
				type: 'text',
				id: 'txtName',
				label: editor.lang.linkfe.anchor.name,
				required: true,
				validate: function() {
					if ( !this.getValue() ) {
						alert( editor.lang.linkfe.anchor.errorName );
						return false;
					}
					return true;
				}
