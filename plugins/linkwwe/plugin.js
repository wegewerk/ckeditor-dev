...
( function() {
	CKEDITOR.plugins.add( 'linkwwe', {
		requires: 'dialog,fakeobjects',
		// jscs:disable maximumLineLength
		lang: 'af,ar,bg,bn,bs,ca,cs,cy,da,de,el,en,en-au,en-ca,en-gb,eo,es,et,eu,fa,fi,fo,fr,fr-ca,gl,gu,he,hi,hr,hu,id,is,it,ja,ka,km,ko,ku,lt,lv,mk,mn,ms,nb,nl,no,pl,pt,pt-br,ro,ru,si,sk,sl,sq,sr,sr-latn,sv,th,tr,tt,ug,uk,vi,zh,zh-cn', // %REMOVE_LINE_CORE%
		// jscs:enable maximumLineLength
		icons: 'anchor,anchor-rtl,linkwwe,unlink', // %REMOVE_LINE_CORE%
		...
		init: function( editor ) {
			...
			if ( CKEDITOR.dialog.isTabEnabled( editor, 'linkwwe', 'advanced' ) )
				allowed = allowed.replace( ']', ',accesskey,charset,dir,id,lang,name,rel,tabindex,title,type]{*}(*)' );
			if ( CKEDITOR.dialog.isTabEnabled( editor, 'linkwwe', 'target' ) )
				allowed = allowed.replace( ']', ',target,onclick]' );

			// Add the link and unlink buttons.
			editor.addCommand( 'linkwwe', new CKEDITOR.dialogCommand( 'linkwwe', {
			...
			editor.setKeystroke( CKEDITOR.CTRL + 76 /*L*/, 'linkwwe' );

			if ( editor.ui.addButton ) {
				editor.ui.addButton( 'LinkWWE', {
					label: editor.lang.linkwwe.toolbar,
					command: 'linkwwe',
					toolbar: 'links,10'
				});
				editor.ui.addButton( 'Unlink', {
					label: editor.lang.linkwwe.unlink,
					command: 'unlink',
					toolbar: 'links,20'
				});
				editor.ui.addButton( 'Anchor', {
					label: editor.lang.linkwwe.anchor.toolbar,
					command: 'anchor',
					toolbar: 'links,30'
				});
			}

			CKEDITOR.dialog.add( 'linkwwe', this.path + 'dialogs/linkwwe.js' );
			CKEDITOR.dialog.add( 'anchor', this.path + 'dialogs/anchor.js' );

			editor.on( 'doubleclick', function( evt ) {
				var element = CKEDITOR.plugins.linkwwe.getSelectedLink( editor ) || evt.data.element;

				if ( !element.isReadOnly() ) {
					if ( element.is( 'a' ) ) {
						evt.data.dialog = ( element.getAttribute( 'name' ) && ( !element.getAttribute( 'href' ) || !element.getChildCount() ) ) ? 'anchor' : 'linkwwe';

						// Pass the link to be selected along with event data.
						evt.data.link = element;
					} else if ( CKEDITOR.plugins.linkwwe.tryRestoreFakeAnchor( editor, element ) ) {
			...
			if ( editor.addMenuItems ) {
				editor.addMenuItems({
					anchor: {
						label: editor.lang.linkwwe.anchor.menu,
						command: 'anchor',
						group: 'anchor',
						order: 1
					},
					...
					removeAnchor: {
						label: editor.lang.linkwwe.anchor.remove,
					...
					linkwwe: {
						label: editor.lang.linkwwe.menu,
						command: 'linkwwe',
						group: 'link',
						order: 1
					},
			...
			if ( editor.contextMenu ) {
				editor.contextMenu.addListener( function( element ) {
					...
					var anchor = CKEDITOR.plugins.linkwwe.tryRestoreFakeAnchor( editor, element );

					if ( !anchor && !( anchor = CKEDITOR.plugins.linkwwe.getSelectedLink( editor ) ) )
						return null;

					var menu = {};

					if ( anchor.getAttribute( 'href' ) && anchor.getChildCount() )
						menu = { linkwwe: CKEDITOR.TRISTATE_OFF, unlink: CKEDITOR.TRISTATE_OFF };
		...
		afterInit: function( editor ) {
			...
			if ( pathFilters ) {
				pathFilters.push( function( element, name ) {
					if ( name == 'a' ) {
						if ( CKEDITOR.plugins.linkwwe.tryRestoreFakeAnchor( editor, element ) || ( element.getAttribute( 'name' ) && ( !element.getAttribute( 'href' ) || !element.getChildCount() ) ) )
							return 'anchor';
					}
				} );
			}
	...
	function protectEmailLinkAsFunction( editor, email ) {
		var plugin = editor.plugins.linkwwe,
	 ...
	 * @singleton
	 */
	CKEDITOR.plugins.linkwwe = {
		/**
		 * Get the surrounding link element of the current selection.
		 *
		 *		CKEDITOR.plugins.linkwwe.getSelectedLink( editor );
		 *
		...
		parseLinkAttributes: function( editor, element ) {
			var href = ( element && ( element.data( 'cke-saved-href' ) || element.getAttribute( 'href' ) ) ) || '',
				compiledProtectionFunction = editor.plugins.linkwwe.compiledProtectionFunction,
			...
			if ( !retval.type ) {
				...
				else if ( urlMatch = href.match( pageRegex ) ) {
					retval.type = 'page';
		...
		getLinkAttributes: function( editor, data ) {
			...
			switch ( data.type ) {
				case 'page':
					set[ 'data-cke-saved-href' ] = data.page.href;
	...
	CKEDITOR.removeAnchorCommand.prototype = {
		exec: function( editor ) {
			var sel = editor.getSelection(),
				...
				anchor;
			if ( sel && ( anchor = sel.getSelectedElement() ) && ( !anchor.getChildCount() ? CKEDITOR.plugins.linkwwe.tryRestoreFakeAnchor( editor, anchor ) : anchor.is( 'a' ) ) )
				anchor.remove( 1 );
			else {
				if ( ( anchor = CKEDITOR.plugins.linkwwe.getSelectedLink( editor ) ) ) {
