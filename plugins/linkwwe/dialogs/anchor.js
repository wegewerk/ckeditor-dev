...
CKEDITOR.dialog.add( 'anchor', function( editor ) {
	...
	return {
		title: editor.lang.linkwwe.anchor.title,
		minWidth: 300,
		minHeight: 60,
		onOk: function() {
		...
		onShow: function() {
			var sel = editor.getSelection(),
				...
				linkElement = fakeSelected ?
					CKEDITOR.plugins.linkwwe.tryRestoreFakeAnchor( editor, fullySelected ) :
		...
		contents: [ {
			id: 'info',
			label: editor.lang.linkwwe.anchor.title,
			accessKey: 'I',
			elements: [ {
				type: 'text',
				id: 'txtName',
				label: editor.lang.linkwwe.anchor.name,
				required: true,
				validate: function() {
					if ( !this.getValue() ) {
						alert( editor.lang.linkwwe.anchor.errorName ); // jshint ignore:line
						return false;
					}
					return true;
				}
